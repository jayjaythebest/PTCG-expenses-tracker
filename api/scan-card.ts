...
function isScanShape(o: unknown): o is ScanShape {
  ...
  // Any of these present as strings → acceptable; we normalise below.
  return ['setCode', 'localId', 'name', 'rarity', 'language'].some(k => typeof r[k] === 'string');
}
...
只根據卡面實際印刷的內容作答，不確定的欄位一律回空字串。只輸出 JSON，不要額外文字。`;
    ...
    // All providers failed → return empty so the client shows its manual-fill path.
